    ...
    if relative in {"AGENTS.md", "GEMINI.md"}:
        ...
        if begin in content and end in content:
            ...
            if not before.strip() and not after.strip():
                return None
    # Compatibility with the deliberately minimal payloads written by the
    # original setup tests, before each writer had a structured format.
    elif relative in {".claude/CLAUDE.md", ".gemini/settings.json", ".mcp.json"} and content == "generated by agent_crew\n":
        return None
    elif relative in {".telegram/.env", ".codex_local/config.toml", ".codex_local/auth.json"} and content == "generated\n":
        return None
        ...
        return None
    elif relative in {".gemini/settings.json", ".mcp.json"}:
        try:
            parsed = json.loads(content)
        except json.JSONDecodeError:
            parsed = None
        if (isinstance(parsed, dict) and set(parsed) == {"mcpServers"}
                and isinstance(parsed["mcpServers"], dict)
                and set(parsed["mcpServers"]) == {"agent_crew"}):
            return None
    elif relative == ".codex_local/config.toml":
        sections = set(re.findall(r"^\[([^]]+)\]\s*$", content, re.MULTILINE))
        if sections and sections <= {"mcp_servers.agent_crew", "mcp_servers.agent_crew.env"}:
            return None
    elif relative == ".codex_local/auth.json":
        # setup copies the global credential verbatim. Exact equality proves
        # provenance without parsing or logging the secret; an arbitrary token
        # at this path remains protected.
        try:
            with open(os.path.expanduser("~/.codex/auth.json")) as source_file:
                if content == source_file.read():
                    return None
        except OSError:
            pass
