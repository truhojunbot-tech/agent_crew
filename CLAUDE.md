...
#### 프로바이더 고갈 시 단계별 처리 (owner 지시 2026-09-20)

프로바이더 한도 소진은 **품질 신호가 아니라 외부 장애**다. 단계마다 다르게 다룬다.

| 단계 | 프로바이더 | 고갈 시 |
|------|-----------|---------|
| implement | codex | **HOLD.** 빈 산출물 뒤에 리뷰어를 붙이면 리뷰 토큰만 탄다. |
| review | claude | **HOLD.** 독립 리뷰 없는 머지는 금지. |
| test | gemini | **생략하고 진행.** 이것 때문에 머지를 막지 않는다. |

- 역할 대체는 어느 경우에도 금지다. claude 가 테스터 자리를 대신 맡는 식의
  silent provider fallback 은 독립성 자체를 없앤다.
- test 를 생략할 때는 **기계적 검증**(pytest 등 LLM 판단이 아닌 실행)을 증거로
  붙인다. 다만 그것이 불가능하다고 멈추지는 않는다.
- 생략했으면 PR 과 이슈에 **"gemini 테스터 단계 미실행(프로바이더 한도) + 대체 증거"**
  를 명시한다. 조용한 생략은 금지 — 나중에 읽는 사람이 게이트가 돌았다고 오해하면
  그 자체가 거짓 보고다.
- 한도가 복구되면 생략했던 검증을 사후에 돌리고 결과를 PASS/FAIL 그대로 올린다.

#### 왜 codex 병목을 감수하는가 (owner 확정 2026-09-21 06:38, 변경 불가)

**claude 주간 한도 보호가 codex 처리량보다 우선한다.** codex 가 하루에 여러 번 한도에
걸려 구현 레인이 멈추는 것은 **의도된 트레이드오프**이지 고쳐야 할 병목이 아니다.

- codex 소진 → **HOLD**. claude 를 implementer 자리에 앉히는 폴백은 금지. 빈 산출물에
  리뷰 토큰을 태우고 claude 한도를 두 번 깎는다.
- **"codex 병목이니 구현을 claude 로 돌리자" 는 제안 자체를 하지 않는다** — 이미 내려진 결정이다.
- 대기 시간에는 프로바이더가 필요 없는 일을 한다: PR 머지, 이슈 정리, 설계 문서화, 증거 수집.

#### 토큰 비용의 실체는 생성량이 아니라 재전송되는 컨텍스트

7일 실측: 함대 전체 `cache_read` 8.38B 중 Alfred 세션 하나가 3.02B(36%). 비용은
**턴 수 × 컨텍스트 크기**로 난다. 따라서 불필요한 재조회, 긴 출력 덤프,
**같은 SHA 재리뷰**를 피한다. 리뷰는 구현이 바뀐 뒤에만 디스패치한다.
