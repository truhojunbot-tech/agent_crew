...
- `provider_context_cleared` — agent_crew sent `/clear` to a pane because its measured context crossed `AGENT_CREW_TOKEN_CLEAR_THRESHOLD` (#297). Carries `task_id`, `pane_id`, `agent`/`provider`, the recorded `context_id`/`context_generation`/`provider_session_id` where one exists, the `context_tokens` and `token_source` that triggered it, `cap_tokens`, `reason="auto_clear_token_threshold"`, and `outcome`.
  - ⛔`outcome` is `attempted`, never `completed`. `tmux send-keys` returning 0 proves the keystrokes reached the pane, not that the provider acted on them; there is no confirmation channel. A non-zero return is recorded as `send_failed`.
  - ⛔The task that was about to be pushed is also marked: `context_reset: true` (so the next resolution bumps the generation and records `fresh` rather than `resume`) plus `auto_cleared_before_push` / `auto_clear_context_tokens` / `auto_clear_token_source`. The first corrects the treatment; the rest let a consumer exclude or stratify auto-cleared rows specifically instead of seeing `fresh` and guessing which of several reasons produced it.
  - An operator's explicit `context_reset` is never relabelled as an auto-clear.
